    ...
    <SectionWrapper className="bg-dark-surface fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-7xl mx-auto">
        {projects.map((project, index) => (
          <div
            key={index}
            className="bg-dark-card p-6 rounded-lg border border-turquoise/20 hover:border-turquoise/40 transition-all duration-300 hover:-translate-y-1 group relative"
          >
            ...
            {project.status === 'live' ? (
              <Link
                ...
                className="text-white/60 hover:text-turquoise transition-colors text-xs flex items-center gap-2 group/link"
              >
                <span className="truncate">{project.url.replace(/^https?:\/\//, '').replace(/\/$/, '')}</span>
                <svg
                  className="w-3.5 h-3.5 opacity-0 group-hover/link:opacity-100 transition-opacity flex-shrink-0"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
